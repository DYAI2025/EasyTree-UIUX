...
export const WorksiteCard: React.FC<WorksiteCardProps> = ({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    try {
      ...
      if (data && data.type === 'EMPLOYEE' && data.employeeId) {
        onMoveEmployee?.(data.employeeId, assignment.id, data.sourceAssignmentId);
      }
    } catch (err) {
  ...
  return (
    <div
      ...
      className={`group relative bg-[#202124] hover:bg-[#292A2E] border rounded-xl p-4 transition-all duration-150 cursor-pointer shadow-md focus-ring ${
        isDragOver
          ? 'border-sky-400 bg-sky-950/40 ring-2 ring-sky-400/50 scale-[1.01]'
          : blockingCount > 0
    ...
    >
      ...
      {isDragOver && (
        <div className="absolute inset-0 z-30 bg-sky-950/85 backdrop-blur-xs border-2 border-dashed border-sky-400 rounded-xl flex flex-col items-center justify-center p-3 text-sky-200 pointer-events-none animate-fadeIn">
          <UserPlus className="w-6 h-6 text-sky-400 mb-1 animate-bounce" />
          <span className="text-xs font-bold font-mono uppercase tracking-wide">
            Mitarbeiter hier ablegen
          </span>
          <span className="text-[10px] text-sky-300/80">Wird sofort diesem Einsatz zugewiesen</span>
        </div>
      )}
      ...
      {/* ASSIGNED EMPLOYEES: Draggable Avatars & Names */}
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-[9px] text-neutral-400/80 font-mono flex items-center gap-1">
            <GripVertical className="w-2.5 h-2.5" /> Ziehen zum Verschieben
          </span>
        </div>
        ...
        {assignedEmployees.length === 0 ? (
          <div className="text-xs text-amber-400 font-medium italic bg-amber-950/40 px-2.5 py-2 rounded-lg border border-amber-800/60 flex items-center gap-2">
            ...
            <span>Noch keine Personen zugewiesen — Person hierher ziehen</span>
          </div>
        ) : (
          <div className="flex flex-wrap items-center gap-1.5">
            {assignedEmployees.map((emp) => (
              <div
                ...
                onClick={(e) => e.stopPropagation()}
                className="flex items-center gap-1 bg-[#171717] hover:bg-[#2A2D35] border border-[#32343A] hover:border-sky-400 rounded-full pl-1 pr-2.5 py-0.5 shadow-xs cursor-grab active:cursor-grabbing transition-all group/emp select-none"
                title={`${emp.firstName} ${emp.lastName} (${emp.role}) — Gedrückt halten & ziehen zum Verschieben auf eine andere Baustelle`}
              >
                <GripVertical className="w-3 h-3 text-neutral-500 group-hover/emp:text-sky-400 shrink-0" />
                <Avatar employee={emp} size="sm" showRoleBadge />
                <span className="text-xs font-semibold text-neutral-200 group-hover/emp:text-white">
                  {emp.firstName}
                </span>
              </div>
            ))}

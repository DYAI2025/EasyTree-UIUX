...
export const WorksiteCard: React.FC<WorksiteCardProps> = ({
  ...
  return (
    <div
      onClick={onCardClick}
      className={`group relative bg-[#202124] hover:bg-[#292A2E] border rounded-xl p-3.5 transition-all duration-150 cursor-pointer shadow-md focus-ring ${
        blockingCount > 0
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            ...
            <span className="text-[10px] text-neutral-400 font-mono bg-[#171717] px-1.5 py-0.2 rounded border border-[#45474D]">
              {worksite.code}
            </span>
          </div>
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="p-1 text-neutral-400 hover:text-white hover:bg-[#32343A] rounded transition focus-ring"
            title="Aktionen"
          >
          ...
          {menuOpen && (
            <div
              ...
              className="absolute right-0 top-6 z-20 w-44 bg-[#171717] border border-[#45474D] rounded-lg shadow-xl p-1 text-xs text-neutral-200"
            >
              <button
                onClick={(e) => {
                  setMenuOpen(false);
                ...
                className="w-full text-left px-2.5 py-1.5 hover:bg-[#32343A] rounded flex items-center gap-2"
              >
              ...
              <button
                ...
                className="w-full text-left px-2.5 py-1.5 hover:bg-rose-950 text-rose-300 rounded flex items-center gap-2"
              >
      ...
      <div className="text-xs text-neutral-300 font-medium bg-[#171717] px-2.5 py-1.5 rounded-md border border-[#292A2E] mb-2.5 line-clamp-1">
      ...
      <div className="mb-2.5">
        ...
        {assignedEmployees.length === 0 ? (
          <div className="text-xs text-amber-400 font-medium italic bg-amber-950/40 p-1.5 rounded border border-amber-800/60 flex items-center gap-1.5">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            {visibleEmployees.map((emp) => (
              <div
                key={emp.id}
                className="flex items-center gap-1.5 bg-[#171717] border border-[#32343A] rounded-full pl-0.5 pr-2 py-0.5"
              >
            ...
            {hiddenEmployeesCount > 0 && (
              <span className="px-2 py-1 text-xs font-semibold bg-[#32343A] text-neutral-300 rounded-full border border-[#45474D]">
      ...
      <div className="flex items-center justify-between gap-2 border-t border-[#292A2E] pt-2 mt-1 text-[11px] text-neutral-400">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          {assignedVehicles.length > 0 && (
            <span className="flex items-center gap-1 text-sky-300 font-medium bg-sky-950/60 px-1.5 py-0.5 rounded border border-sky-800">
          ...
          {assignedEquipment.length > 0 && (
            <span className="flex items-center gap-1 text-purple-300 font-medium bg-purple-950/60 px-1.5 py-0.5 rounded border border-purple-800">
        ...
        {/* Conflict Badge */}
        {blockingCount > 0 ? (
          <span className="px-2 py-0.5 text-[10px] font-bold bg-rose-900 text-rose-100 rounded border border-rose-700 flex items-center gap-1 shrink-0 animate-pulse">
            <AlertTriangle className="w-3 h-3 text-rose-300" />
            {blockingCount} Konflikt
          </span>
        ) : warningCount > 0 ? (
          <span className="px-2 py-0.5 text-[10px] font-medium bg-amber-950 text-amber-300 rounded border border-amber-800 flex items-center gap-1 shrink-0">
